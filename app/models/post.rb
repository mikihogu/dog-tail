class Post < ApplicationRecord
  ...
  def save_notification_comment!(current_member, comment_id, member_id)
    # コメントは複数回することが考えられるため、１つの投稿に複数回通知する
    notification = current_member.comment.notifications.new(
      post_id: id,
      comment_id: comment_id,
    )
    # 自分の投稿に対するコメントの場合は、通知済みとする
    if notification.comment.member_id == notification.post.member_id
